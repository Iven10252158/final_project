    ...
    <div class="row border p-3 d-flex justify-content-center" v-for="item in orderForm.products" :key="item">
      ...
      <div class="col-12 col-md-6">
        ...
        <div class="row" v-if="orderForm">
          <div class="col-4 h6 text-secondary">訂單編號</div>
          <div class="col-8">{{orderForm.id}}</div>
          <div class="col-4 h6 text-secondary">付款金額</div>
          <div class="col-8">NT$ {{$filters.currency(orderForm.total)}}</div>
          <div class="col-4 h6 text-secondary">付款狀態</div>
          <div class="col-8" :class="{'text-primary':orderForm.is_paid}">{{orderForm.is_paid? '付款成功':'尚未付款'}}</div>
        </div>
        <div class="d-flex" v-if="orderForm.is_paid">
          <div class="col-4 h6 text-secondary">訂單成立</div>
